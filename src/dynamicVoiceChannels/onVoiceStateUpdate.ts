import type { Client, StageChannel, VoiceChannel, VoiceState } from 'discord.js';
...
const deleteChannel = async (tempChannel: VoiceChannel | StageChannel) => {
...
const createChannel = async (newVoiceState: VoiceState): Promise<void> => {
    ...
    await createdChannel.permissionOverwrites.create(newVoiceState.member.id, { MANAGE_CHANNELS: true });
};
